...
public class FacesScaffold extends BaseFacet implements ScaffoldProvider
{
   ...
   private static final String MEDIATOR_TEMPLATE = "scaffold/faces/Mediator.jv";
   private static final String REPOSITORY_TEMPLATE = "scaffold/faces/Repository.jv";
   ...
   private static final String CREATE_TEMPLATE = "scaffold/faces/edit.xhtml";
   ...
   private static final String INPUT_TEMPLATE = "scaffold/faces/input.xhtml";
   ...
   protected CompiledTemplateResource mediatorTemplate;
   protected CompiledTemplateResource repositoryTemplate;
   ...
   protected CompiledTemplateResource inputTemplate;
            ...
            final boolean overwrite)
   {
      ...
      try
      {
         ...
         viewBean.setPackage(java.getBasePackage() + ".interfaces.web");
         result.add(ScaffoldUtil.createOrOverwrite(this.prompt, java.getJavaResource(viewBean), viewBean.toString(), overwrite));
         
         
         // TODO adrianob Create the Mediator for this entity
         JavaClass mediator = JavaParser.parse(JavaClass.class, this.mediatorTemplate.render(context));
         mediator.setPackage(java.getBasePackage() + ".domain.service");
         result.add(ScaffoldUtil.createOrOverwrite(this.prompt, java.getJavaResource(mediator), mediator.toString(), overwrite));
         
         
         // TODO adrianob Create the Repository for this entity
         JavaClass repository = JavaParser.parse(JavaClass.class, this.repositoryTemplate.render(context));
         repository.setPackage(java.getBasePackage() + ".infrastructure.persistence.hibernate");
         result.add(ScaffoldUtil.createOrOverwrite(this.prompt, java.getJavaResource(repository), repository.toString(), overwrite));
         ...
         result.add(ScaffoldUtil.createOrOverwrite(this.prompt,
                  web.getWebResource(targetDir + "/" + ccEntity + "/" + ccEntity + "Edit.xhtml"),
         ...
         result.add(ScaffoldUtil.createOrOverwrite(this.prompt,
                  web.getWebResource(targetDir + "/" + ccEntity + "/" + ccEntity + "View.xhtml"),
                  this.viewTemplate.render(context), overwrite));

         // Generate search
         this.searchMetawidget.setValue(StaticFacesUtils.wrapExpression(beanName + ".filter"));
         ...
         result.add(ScaffoldUtil.createOrOverwrite(this.prompt,
                  web.getWebResource(targetDir + "/" + ccEntity + "/" + ccEntity + "Search.xhtml"),
   ...
   public List<Resource<?>> generateIndex(String targetDir, final Resource<?> template, final boolean overwrite)
   {
      ...
      // TODO adrianob
      result.add(ScaffoldUtil.createOrOverwrite(this.prompt, web.getWebResource(targetDir + "/resources/components/property/input.xhtml"),
              this.inputTemplate.render(context), overwrite));
   ...
   protected void loadTemplates()
   {
      ...
      if (this.mediatorTemplate == null)
      {
         this.mediatorTemplate = this.compiler.compile(MEDIATOR_TEMPLATE);
         //String template = Streams.toString(this.mediatorTemplate.getSourceTemplateResource().getInputStream());
         //this.mediatorTemplateQbeMetawidgetIndent = parseIndent(template, "@{qbeMetawidget}");
      }
      
      if (this.repositoryTemplate == null)
      {
         this.repositoryTemplate = this.compiler.compile(REPOSITORY_TEMPLATE);
         //String template = Streams.toString(this.repositoryTemplate.getSourceTemplateResource().getInputStream());
         //this.backingBeanTemplateQbeMetawidgetIndent = parseIndent(template, "@{qbeMetawidget}");
      }
      ...
      {
         this.inputTemplate = this.compiler.compile(INPUT_TEMPLATE);
      }
   }
            ...
            throws IOException
   {
      ...
      for (Resource<?> resource : web.getWebResource(targetDir + "/").listResources(filter))
      {
         ...
         // TODO adrianob
         outcomeTargetLink.putAttribute("outcome", targetDir + "/" + resource.getName() + "/" + resource.getName() + "Search");
